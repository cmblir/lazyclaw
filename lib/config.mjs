...
import { defaultConfigDir as resolveConfigDir } from './config_dir.mjs';

export function configPath() {
  const override = process.env.POMPOS_CONFIG_DIR;
  const dir = override || resolveConfigDir();
...
export function readConfig() {
  ...
  try {
    ...
    process.stderr.write(
      `\npompos: config.json is present but not valid JSON — refusing to ` +
...
export function _resolveBaseUrl(provider) {
  return {
    anthropic: process.env.POMPOS_ANTHROPIC_BASE_URL,
    openai:    process.env.POMPOS_OPENAI_BASE_URL,
    gemini:    process.env.POMPOS_GEMINI_BASE_URL,
  }[provider] || undefined;
}
...
export function readVersionFromRepo() {
  ...
  //      entrypoint once installed via `npm i -g pompos`).
