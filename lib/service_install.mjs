...
export function servicePaths(name, { home = os.homedir(), configDir } = {}) {
  const cfg = configDir || path.join(home, '.pompos');
  return {
    launchd: path.join(home, 'Library', 'LaunchAgents', `com.pompos.${name}.plist`),
    systemd: path.join(home, '.config', 'systemd', 'user', `pompos-${name}.service`),
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.pompos.${xmlEscape(name)}</string>
...
export function installService(spec, depsIn = {}) {
  ...
  if (backend === 'systemd') {
    ...
    const r = deps.spawnSync('systemctl', ['--user', 'enable', '--now', `pompos-${name}.service`], { encoding: 'utf8' });
...
export function uninstallService(spec, depsIn = {}) {
  ...
  if (backend === 'systemd') {
    deps.spawnSync('systemctl', ['--user', 'disable', '--now', `pompos-${name}.service`], { stdio: 'ignore' });
...
export function serviceStatus(spec, depsIn = {}) {
  ...
  if (backend === 'systemd') {
    ...
    const r = deps.spawnSync('systemctl', ['--user', 'is-active', `pompos-${name}.service`], { encoding: 'utf8' });
