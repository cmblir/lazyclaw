...
// Capture console.warn while running fn, with POMPOS_DEBUG forced to `dbg`.
function capWarn(dbg, fn) {
  _resetNativeHint();
  const prevDbg = process.env.POMPOS_DEBUG;
  if (dbg) process.env.POMPOS_DEBUG = '1'; else delete process.env.POMPOS_DEBUG;
  const seen = [];
  const orig = console.warn; console.warn = (...a) => seen.push(a.join(' '));
  try { fn(); } finally {
    console.warn = orig;
    if (prevDbg === undefined) delete process.env.POMPOS_DEBUG; else process.env.POMPOS_DEBUG = prevDbg;
  }
  return seen;
}

test('_warnIndexFailure stays SILENT for the user (no POMPOS_DEBUG) — internals never hit the screen', () => {
...
test('_warnIndexFailure prints the rebuild hint ONCE under POMPOS_DEBUG (no per-op spam)', () => {
...
test('_warnIndexFailure warns per-op for ordinary errors under POMPOS_DEBUG', () => {
  const seen = capWarn(true, () => {
    _warnIndexFailure('indexSkill failed', new Error('database is locked'));
    _warnIndexFailure('indexSkill failed', new Error('database is locked'));
  });
