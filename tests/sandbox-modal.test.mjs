...
test('idleWakeUrl encodes app + token for 30-min hibernation wake hook', () => {
  const url = idleWakeUrl({ app: 'pompos-worker', token: 'tok123' });
  assert.match(url, /^https:\/\/.*modal\.run\/wake\?app=pompos-worker&token=tok123$/);
});
