...
pompos splits two provider slots: **`provider`** (chat — the hot path) and **`trainer`** (the learning loop — skill synthesis, reflection, the user model). They are independent, so a Claude Pro/Max subscription can power the learning while chat runs anywhere — or both run on one backend.
...
# 2) point a listener at it (Slack Socket Mode: SLACK_BOT_TOKEN + SLACK_APP_TOKEN in ~/.pompos/.env)
pompos slack listen                 # forwards inbound to the daemon, replies in-thread
pompos slack listen --provider orchestrator   # …and orchestrate the reply
pompos slack listen --daemon-url http://127.0.0.1:19600   # non-default daemon

pompos channels                     # view configured channels
...
**One process, every channel:** `pompos gateway` runs the daemon core *and* your configured channel transports (Slack Socket Mode / Telegram long-poll / Matrix sync) in a single process — and because the channels live in-process, `/handoff` can notify the target channel with a resume marker (a failed notify rolls the handoff back).

```bash
pompos gateway                          # daemon core + every enabled channel, one process
pompos gateway --channels slack         # explicit channel set
pompos service install gateway          # …and keep it alive across reboots
```

The gateway is **authenticated by default**: it mints a bearer token on first run and persists it to `~/.pompos/gateway.token` (0600, never logged). Its own channels use it automatically; external callers read it from the file (`--auth-token`/`--no-auth` to override).
...
```bash
pompos service install                 # daemon only: launchd (macOS) · systemd user unit (Linux) · pidfile fallback
pompos service status
pompos service uninstall
```

The backend is auto-detected (override with `--backend launchd|systemd|fallback`); flags pass through, e.g. `pompos service install gateway --port 19600 --auth-token "$TOK" --channels slack`.
...
pompos orchestrator on            # route chats through the pipeline
```
...
- **Skills** — markdown instruction bundles composed into the system prompt. `pompos skills starter` installs the bundled pack (`concise` · `korean` · `commit-message` · `code-review` · `channel-style` · `summarize` · `explain` · `debug-coach`); `pompos skills install <user>/<repo>` pulls more from GitHub; `/skills` picks one in chat.
- **Durable recall** — one SQLite + FTS5 index over sessions, skills, trajectories, and memory; rebuildable from the corpus (`pompos index rebuild`). Optionally blend in embedding similarity (`cfg.recall.embeddings`, off by default; OpenAI/Gemini key or a local Ollama model — the $0 path stays pure FTS5); `pompos index embed` backfills the vectors.
